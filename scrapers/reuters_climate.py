"""
Reuters Climate & Energy Scraper
=================================
Website: https://www.reuters.com/sustainability/climate-energy/

Selectors:
- Articles: <a href="/sustainability/climate-energy/..."> with date in URL (YYYY-MM-DD)
- Also catches: /sustainability/cop/, /business/energy/, /world/, /legal/
...
Logic:
1. Visit climate-energy section page with browser cookies
...
NEWS_URL = 'https://www.reuters.com/sustainability/climate-energy/'
...
def extract_date_from_url(url):
    """Extract date from URL like /sustainability/climate-energy/article-title-2025-12-19/"""
...
def get_article_links(soup):
    """Extract article links from climate-energy section page"""
    articles = []
    seen = set()
    ...
    # Climate/sustainability related URL patterns
    valid_patterns = [
        '/sustainability/climate-energy/',
        '/sustainability/cop/',
        '/sustainability/boards-policy-regulation/',
        '/business/energy/',
        '/world/',
        '/legal/',
    ]
    ...
    for link in title_links:
        href = link.get('href', '')
        if not any(href.startswith(p) for p in valid_patterns):
            continue
        if href in seen:
            continue
    ...
    # Method 3: Fallback - scan ALL links with climate/sustainability URLs
...
def main():
    print("=" * 60)
    print(f"Reuters Climate & Energy Scraper - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 60)
    
    # Load existing links
    existing_links = get_existing_links()
    existing_from_source = [l for l in existing_links if 'reuters.com' in l and ('sustainability' in l or 'climate' in l)]
    print(f"Already scraped from Reuters Climate: {len(existing_from_source)} articles")
    ...
    try:
        ...
        if response.status_code == 401:
            print("ERROR: 401 Unauthorized - Cookies expired!")
            print("Update the 'cookies' dict from browser DevTools > Application > Cookies")
            return
    ...
    scraped_articles = fetch_articles_parallel(
    ...
    if scraped_articles:
        ...
        print(f"\n✓ Saved {len(scraped_articles)} articles to {CSV_FILE}")
    
    print(f"\nScraped {len(scraped_articles)} new articles")
